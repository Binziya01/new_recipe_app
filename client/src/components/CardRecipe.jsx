...
const CardRecipe = ({item}) => {
    ...
    const categoryStyles = {
        ...
        Sides: {backgroundColor:"#f0f5c4", color:"#59871f"},
  ...
  return (
    <div className='container mx-auto flex justify-center md:justify-start'>
        ...
        <div className='bg-white relative shadow-lg hover:shadow-xl transition duration-500 rounded-lg'>
            <img src={item?.thumbnailImage} alt="thumbnail_image" className='rounded-t-lg' />

            <div className='py-6 px-5 rounded-lg bg-white'>
                <Link to={`/recipes/${item._id}`}>
                    <h1 className='text-gray-700 font-bold text-2xl mb-8 hover:text-gray-900 hover:cursor-pointer'>{item?.recipeName}</h1>
                </Link>
            ...
            {/* <div className='absolute top-2 right-2 py-2 px-4 bg-white rounded-lg'>
                <span className='font-medium text-black'>{item?.more.difficulty}</span>
            </div> */}
        </div>
